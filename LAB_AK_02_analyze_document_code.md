...
1. Use a drag-and-drop operation to add the following files from the SOLUTION EXPLORER view to the Chat context: **JsonData.cs**, **JsonLoanRepository.cs**, and **JsonPatronRepository.cs**.
    ...
    ```
    for example 
    #codebase Explain how loan data is loaded from JSON and how patrons are handled.
    ```
...
1. Ensure that you have the **Solution Explorer** view open.
    ...
    i.e 
    “These instructions are written for Visual Studio. We are using VS Code, so we’ll use Explorer + Copilot Chat instead.”
...
1. To run the application, right-click **Library.Console**, select **Debug**, and then select **Start New Instance**.
    ...
    Open a terminal or the integrated terminal in Visual Studio Code and navigate to the Library.Console project directory:


```cd "c:\Users\henim\Documents\folder_VS_Copilot\AZ2007LabAppM2\AccelerateDevGHCopilot\src\Library.Console"```
...
Run the Library.Console project:


```dotnet run```
This will start the console application, and you can interact with it as intended.


2. When prompted for a patron name, type **One** and then press Enter.

    You should see a list of patrons that match the search query.

    > **NOTE**: The application uses a case-sensitive search process.

3. At the "Input Options" prompt, type **2** and then press Enter.
...
2. At the "Input Options" prompt, type **r** and then press Enter.

    Entering **r** returns the book.

3. Verify that the message "Book was successfully returned." is displayed.
...
4. To begin a new search, type **s** and then press Enter.

5. When prompted for a patron name, type **One** and then press Enter.

6. At the "Input Options" prompt, type **2** and then press Enter.

7. Verify that first book loan is marked **Returned: True**.

8. At the "Input Options" prompt, type **q** and then press Enter.

9. Stop the debug session.
